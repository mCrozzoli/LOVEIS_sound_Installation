...
from MiBand2.constants import UUIDS, AUTH_STATES, ALERT_TYPES, QUEUE_TYPES
    ...
    def handleNotification(self, hnd, data):
        ...
        # It can trigger the communication.
            ...
            if data[:3] == b'\x10\x01\x01':
                ...
                print("Unexpected data on handle "
                      + str(hnd) + ": " + data.hex())
                return
                ...
                while i < len(data):
                    ...
                    if self.device.outfile:
                        self.device.outfile.write(
                            f"{timestamp.strftime('%d.%m.%Y - %H:%M')},{category},{intensity},{steps},{heart_rate}\n")
...
class MiBand2(Peripheral):
    ...
    def _auth_previews_data_notif(self, enabled):
        if enabled:
            ...
            self._log.error(
                "Something went wrong while changing the Fetch and Activity notifications status...")
    ...
    def create_date_data(date):
        data = struct.pack('hbbbbbbbxx', date.year, date.month, date.day,
                           date.hour, date.minute, date.second, date.weekday(), 0)
        return data
    ...
    def get_revision(self):
        ...
        revision = struct.unpack(
            '9s', data[-9:])[0] if len(data) == 9 else None
        return revision
    ...
    def get_hrdw_revision(self):
        ...
        data = char.read()
        revision = struct.unpack(
            '8s', data[-8:])[0] if len(data) == 8 else None
        return revision
    ...
    def set_heart_monitor_sleep_support(self, enabled=True, measure_minute_interval=1):
        ...
        if enabled:
            ...
            self._char_heart_ctrl.write(
                b'\x14' + bytes([measure_minute_interval]), True)
